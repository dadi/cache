...
describe('RedisCache', () => {
  describe('set', () => {
    ...
    it(
      'should add to Redis cache when a String is passed',
    ...
    it(
      'should add to Redis cache and save metadata when a String and options.metadata are passed',
      sinon.test(function (done) {
        const client = new RedisMock()
        this.stub(noderedis, 'createClient').returns(client)
        const spy = this.spy(client, 'set')
    ...
    it(
      'should add to Redis cache when a Buffer is passed',
      sinon.test(function (done) {
        const client = new RedisMock()
        this.stub(noderedis, 'createClient').returns(client)
        const spy = this.spy(client, 'set')
        ...
        cache.set('key-buffer', Buffer.from('data'))
    ...
    it(
      'should add to Redis cache and save metadata when a Buffer and options.metadata are passed',
      sinon.test(function (done) {
        const client = new RedisMock()
        this.stub(noderedis, 'createClient').returns(client)
        const spy = this.spy(client, 'set')
        ...
        cache.set('key-buffer', Buffer.from('data'), { metadata }).then(() => {
          // check params passed to SET
          spy.called.should.eql(true)
          spy.firstCall.args[0].indexOf('key-buffer').should.be.above(-1)
    ...
    it(
      'should add to Redis cache when a Stream is passed',
      sinon.test(function (done) {
        const client = new RedisMock()
        this.stub(noderedis, 'createClient').returns(client)
        const spy = this.spy(client, 'set')
    ...
    it(
      'should add to Redis cache and save metadata when a Stream and options.metadata are passed',
      sinon.test(function (done) {
        const client = new RedisMock()
        this.stub(noderedis, 'createClient').returns(client)
        const spy = this.spy(client, 'set')
    ...
    it(
      'should set and return binary data',
      sinon.test(function (done) {
        this.timeout(5000)
        // const client = new RedisMock()
        ...
        var catStream = fs.createReadStream(
          require('path').join(__dirname, './../cat.png'),
          {
            encoding: null
          }
        )
        ...
        cache.on('ready', () => {
          cache.set('cat-stream', catStream).then(() => {
            cache.get('cat-stream').then(outStream => {
              ...
              var out = fs.createWriteStream('./../cat2.png')
    ...
    it.skip('should fallback to file cache when `set` is called and Redis is not connected', function (done) {
      ...
      cache = new Cache({
        directory: { enabled: false, path: './cache', extension: 'json' },
        redis: { enabled: true, host: '127.0.0.1', port: 6378 }
      })
      ...
      cache
        ...
        .then(() => {
          ...
          setTimeout(function () {
            fs.stat(
              path.resolve(
                path.join(cache.cacheHandler.directory, 'key1.json')
              ),
              (_, stats) => {
                should.exist(stats)
                done()
  ...
  describe('get', function () {
    ...
    it.skip(
      'should fallback to file cache when `get` is called and Redis is not connected',
      sinon.test(function () {
        cache = new Cache({
          directory: { enabled: false, path: './cache', extension: 'json' },
          redis: { enabled: true, host: '127.0.0.1', port: 6378 }
    ...
    it(
      'should reject if the key cannot be found',
      sinon.test(function () {
        const client = new RedisMock()
    ...
    it(
      ...
      sinon.test(function (done) {
        ...
        cache
          .get('key1')
          .then(stream => {
            toString(stream, function (_, data) {
  ...
  describe('getMetadata', function () {
    it(
      'should return and parse metadata',
      sinon.test(function (done) {
        const client = new RedisMock()
        this.stub(noderedis, 'createClient').returns(client)
        // const spy = this.spy(client, 'set')
    ...
    it(
      'should return null when there is no metadata for the given key',
      sinon.test(function (done) {
        const client = new RedisMock()
        this.stub(noderedis, 'createClient').returns(client)
  ...
  describe('flush', function () {
    it(
      'should delete the keys that match a given pattern, along with any associated metadata',
      sinon.test(function (done) {
