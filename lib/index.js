...
Cache.prototype.createRedisCache = function (options, ttl) {
  ...
  handler.on('end', () => {
    // should fire only on graceful dc
    this.emit('message', 'REDIS disconnecting')
    this.cacheHandlers.redis = null
  })

  handler.on('reconnecting', function (attempt) {
    // every attempt
  ...
  handler.on('fail', (cmd, key, data) => {
    this.emit(
      'message',
      'REDIS connection failed. Falling back to filesystem caching at ' +
        path.resolve(this.options.directory.path)
    )
