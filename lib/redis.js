...
RedisCache.prototype.initialise = function initialise (options) {
  ...
  if (options.cluster) {
    ...
    redisClient = noderedis.createClient(options.port, options.host, {
      detect_buffers: true,
      retryStrategy
    })
  }
  ...
  redisClient.on('reconnecting', attempt => {
    ...
    this.emit(
      'reconnecting',
      this.isClustered ? attemptNumber : attemptNumber.attempt
    )
    this.emit(
      'message',
      'reconnect attempt #' + this.isClustered
        ? attemptNumber
...
RedisCache.prototype.get = function get (key, options) {
  ...
  return new Promise((resolve, reject) => {
    this.redisClient.exists(key, (_, exists) => {
      if (exists > 0) {
        if (this.isClustered) {
...
RedisCache.prototype.getMetadata = function getMetadata (key) {
  ...
  return new Promise((resolve, reject) => {
    this.redisClient.exists(metadataKey, (_, exists) => {
      if (exists === 0) {
        return resolve(null)
      }
...
RedisCache.prototype.deleteKeys = function (keys) {
  return new Promise((resolve, reject) => {
    if (keys.length > 0) {
      ...
      keys.forEach(key => {
        this.redisClient.del(key, (_, result) => {
          if (++i === keys.length) {
            return resolve('')
          }
        })
