 ...
 * @returns {Promise.<String, Error>} A promise that returns an empty String if successful, otherwise an Error
 */
FileCache.prototype.flush = function flush(filepath) {
  return new Promise((resolve, reject) => {
    const cachePath = path.join(this.directory, filepath || '')
    const exists = fs.existsSync(cachePath)

    if (!exists) reject('directory doesn\'t exist')
    ...
    if (fs.statSync(cachePath).isDirectory()) {
      ...
      files.forEach((filename) => {
        const file = path.join(cachePath, filename)
        fs.unlinkSync(file)
