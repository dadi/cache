...
function FileCache (options) {
  ...
  this.directoryChunkSize =
    options.directoryChunkSize && Number.isFinite(options.directoryChunkSize)
      ? options.directoryChunkSize
      : 0
...
FileCache.prototype.get = function get (key, options) {
  ...
  let errorNotFound = new Error('The specified key does not exist')
  ...
  return this.db.then(db => {
    ...
    if ((Date.now() - lastModified) / 1000 > ttl) {
      ...
      return Promise.reject(new Error('The specified key has expired'))
    }
...
FileCache.prototype.getCachePath = function getCachePath (key, options) {
  ...
  if (options.directory && options.directory.extension) {
    extension =
      options.directory.extension[0] === '.'
        ? options.directory.extension
        : '.' + options.directory.extension
  } else {
  ...
  return this.db
    .then(db => {
      ...
      matches.forEach(match => {
        ...
        if (
          match.lastModified &&
          (Date.now() - match.lastModified) / 1000 > this.ttl
        ) {
          deletionQueue.push(
...
FileCache.prototype.startDatabase = function () {
  return new Promise((resolve, reject) => {
    let db = new Loki(path.join(this.directory, 'db.json'), {
      autoload: true,
      autoloadCallback: () => {
